...
### Example 2: Domain-Specific Validation MCP Server

**Scenario**: Create an MCP server to validate score submissions and game data for the high-scores leaderboard application.
...
// Add to VS Code settings.json
{
  "github.copilot.chat.mcp.servers": {
    ...
    // High Scores Validator - Domain-specific validation
    "highscores-validator": {
      "command": "node",
      "args": ["./tools/highscores-validator/index.js"],
      "cwd": "${workspaceFolder}"
    }
  }
}
...
**Step 2: Implement Validation MCP Server**
```typescript
// File: tools/highscores-validator/index.js

import { MCPServer } from '@modelcontextprotocol/server';
import fs from 'fs/promises';

const server = new MCPServer({
  name: 'highscores-validator',
  version: '1.0.0',
  description: 'Validation tools for high scores leaderboard application'
});

// Tool 1: Validate score submission data
server.registerTool({
  name: 'validate_score_submission',
  description: 'Validate score submission JSON against business rules',
  parameters: {
    type: 'object',
    properties: {
      score_data: {
        type: 'object',
        description: 'Score submission object with game_name, player_name, score, timestamp'
      }
    },
    required: ['score_data']
  },
  handler: async ({ score_data }) => {
    const errors = [];
    
    // Validate required fields
    if (!score_data.game_name || typeof score_data.game_name !== 'string') {
      errors.push('game_name is required and must be a string');
    } else if (score_data.game_name.length < 1 || score_data.game_name.length > 100) {
      errors.push('game_name must be between 1 and 100 characters');
    }
    
    if (!score_data.player_name || typeof score_data.player_name !== 'string') {
      errors.push('player_name is required and must be a string');
    } else if (score_data.player_name.length < 1 || score_data.player_name.length > 50) {
      errors.push('player_name must be between 1 and 50 characters');
    }
    
    // Validate score (must be non-negative integer)
    if (typeof score_data.score !== 'number') {
      errors.push('score must be a number');
    } else if (score_data.score < 0) {
      errors.push('score must be non-negative (>= 0)');
    } else if (!Number.isInteger(score_data.score)) {
      errors.push('score must be an integer');
    }
    
    // Validate timestamp format (ISO 8601)
    if (score_data.timestamp) {
      const timestamp = new Date(score_data.timestamp);
    ...
    return {
      ...
      message: errors.length === 0 
        ? `✅ Score submission is valid` 
        : `❌ Found ${errors.length} validation error(s)`
    };
  }
});

// Tool 2: Check score range for game
server.registerTool({
  name: 'check_score_range',
  description: 'Verify if a score is within reasonable range for a game',
  parameters: {
    type: 'object',
    properties: {
      game_name: { type: 'string', description: 'Name of the game' },
      score: { type: 'number', description: 'Score value to validate' },
      max_reasonable_score: { 
        type: 'number', 
        description: 'Maximum reasonable score (optional, defaults to 1,000,000)' 
      }
    },
    required: ['game_name', 'score']
  },
  handler: async ({ game_name, score, max_reasonable_score = 1000000 }) => {
    const warnings = [];
    
    if (score > max_reasonable_score) {
      warnings.push(`Score ${score} exceeds reasonable maximum (${max_reasonable_score})`);
    }
    
    if (score === 0) {
      warnings.push('Score is exactly 0 - may indicate incomplete game session');
    }
    ...
    return {
      ...
      message: warnings.length === 0
        ? `✅ Score ${score} is within reasonable range for ${game_name}`
        : `⚠️ Score ${score} has ${warnings.length} warning(s)`
    };
  }
});

// Tool 3: Validate player name format
server.registerTool({
  name: 'validate_player_name',
  description: 'Validate player name against naming conventions',
  parameters: {
    type: 'object',
    properties: {
      player_name: { type: 'string', description: 'Player name to validate' }
    },
  ...
  handler: async ({ player_name }) => {
    ...
    if (!validPattern.test(player_name)) {
      errors.push('Player name can only contain letters, numbers, spaces, hyphens, and underscores');
    }
    
    // Check for profanity (basic example)
    const profanityList = ['badword1', 'badword2']; // In real app, use comprehensive list
    const lowerName = player_name.toLowerCase();
    const foundProfanity = profanityList.find(word => lowerName.includes(word));
    if (foundProfanity) {
      errors.push('Player name contains inappropriate language');
    }
    ...
    return {
      ...
      message: errors.length === 0
        ? `✅ Player name "${player_name}" is valid`
        : `❌ Player name "${player_name}" has ${errors.length} error(s)`
    };
  }
});
...
@highscores-validator validate_score_submission {
  "game_name": "Pac-Man",
  "player_name": "HighScoreHero",
  "score": 125000,
  "timestamp": "2026-02-04T10:30:00Z"
}

# Check if score is reasonable
@highscores-validator check_score_range {
...
@highscores-validator validate_player_name {"player_name": "RetroGamer_2026"}
```

### Example 3: Analytics & Statistics MCP Server

**Scenario**: Create an MCP server to compute player statistics and detect score anomalies for the leaderboard.

**Step 1: Configure Analytics MCP Server**
```json
// Add to VS Code settings.json
{
  "github.copilot.chat.mcp.servers": {
    ...
    // High Scores Analytics - Statistics and analysis
    "highscores-analytics": {
      "command": "node",
      "args": ["./tools/highscores-analytics/index.js"],
      "cwd": "${workspaceFolder}"
    }
  }
}
...
**Step 2: Implement Analytics MCP Server**
```typescript
// File: tools/highscores-analytics/index.js

import { MCPServer } from '@modelcontextprotocol/server';
import fs from 'fs/promises';

const server = new MCPServer({
  name: 'highscores-analytics',
  version: '1.0.0',
  description: 'Analytics and statistics tools for high scores leaderboard'
});

// Helper: Load seed data (simulates database query)
async function loadScoresFromSeedData() {
  try {
    const data = await fs.readFile('./shared/seed-data.json', 'utf-8');
...
// Tool 1: Calculate player statistics
server.registerTool({
  name: 'calculate_player_stats',
  description: 'Calculate comprehensive statistics for a player (matches PlayerStats model)',
  parameters: {
    type: 'object',
    properties: {
      player_name: { type: 'string', description: 'Player name to analyze' },
      game_name: { 
        type: 'string', 
        description: 'Optional: Filter by specific game',
        optional: true 
      }
    },
    required: ['player_name']
  },
  handler: async ({ player_name, game_name }) => {
    const seedData = await loadScoresFromSeedData();
    let playerScores = seedData.scores.filter(s => s.player_name === player_name);
    
    if (game_name) {
      playerScores = playerScores.filter(s => s.game_name === game_name);
    }
    ...
    if (playerScores.length === 0) {
      return {
        ...
        total_games: 0,
        total_scores: 0,
        message: `No scores found for player "${player_name}"`
      };
    }
    
    // Calculate statistics (matches PlayerStats model from types)
    const scores = playerScores.map(s => s.score);
    const total_score = scores.reduce((sum, s) => sum + s, 0);
    const avg_score = total_score / scores.length;
    const max_score = Math.max(...scores);
    const min_score = Math.min(...scores);
    
    // Get unique games
    const games = [...new Set(playerScores.map(s => s.game_name))];
    
    return {
      player_name: player_name,
      total_games: games.length,
      total_scores: playerScores.length,
      total_score: total_score,
      average_score: Math.round(avg_score),
      highest_score: max_score,
      lowest_score: min_score,
      games_played: games,
      message: `📊 Stats for ${player_name}: ${playerScores.length} scores across ${games.length} game(s)`
    };
  }
});

// Tool 2: Find top scores for a game
server.registerTool({
  name: 'find_top_scores',
  description: 'Get top N scores for a specific game (matches leaderboard API)',
  parameters: {
    type: 'object',
    properties: {
      game_name: { type: 'string', description: 'Name of the game' },
  ...
  handler: async ({ game_name, limit = 10 }) => {
    const seedData = await loadScoresFromSeedData();
    const gameScores = seedData.scores.filter(s => s.game_name === game_name);
    
    if (gameScores.length === 0) {
      return {
        game_name: game_name,
        top_scores: [],
        message: `No scores found for game "${game_name}"`
      };
    }
    ...
    return {
      game_name: game_name,
      total_scores: gameScores.length,
      top_scores: topScores,
      limit: limit,
      message: `🏆 Top ${topScores.length} scores for ${game_name}`
    };
  }
});

// Tool 3: Detect score anomalies
server.registerTool({
  name: 'detect_score_anomalies',
  description: 'Identify suspicious scores that may indicate cheating or data issues',
  parameters: {
    type: 'object',
    properties: {
      game_name: { 
        type: 'string', 
        description: 'Optional: Analyze specific game',
        optional: true 
      },
      threshold_stddev: { 
        type: 'number', 
        description: 'Standard deviation threshold (default: 3)',
        optional: true 
      }
    },
    required: []
  },
  handler: async ({ game_name, threshold_stddev = 3 }) => {
    const seedData = await loadScoresFromSeedData();
    let scores = seedData.scores;
    
    if (game_name) {
      scores = scores.filter(s => s.game_name === game_name);
    }
    
    if (scores.length < 3) {
      return {
        anomalies: [],
        message: 'Insufficient data for anomaly detection (need at least 3 scores)'
      };
    }
    ...
    // Find anomalies (scores beyond threshold * stddev from mean)
    const anomalies = scores
      .map(s => ({
        ...s,
        z_score: (s.score - mean) / stddev,
        deviation: Math.abs(s.score - mean)
      }))
      .filter(s => Math.abs(s.z_score) > threshold_stddev)
      .sort((a, b) => Math.abs(b.z_score) - Math.abs(a.z_score));
    
    return {
      game_name: game_name || 'All Games',
      total_scores_analyzed: scores.length,
      ...
      anomalies: anomalies.map(a => ({
        ...
        deviation_from_mean: Math.round(a.deviation),
        flag: a.z_score > 0 ? '⚠️ Unusually HIGH' : '⚠️ Unusually LOW'
      })),
      message: anomalies.length > 0
        ? `🚨 Found ${anomalies.length} anomalous score(s)`
        : `✅ No anomalies detected (all scores within ${threshold_stddev}σ)`
    };
  }
});
...
@highscores-analytics calculate_player_stats {"player_name": "AAA"}

# Get top 10 scores for a game
@highscores-analytics find_top_scores {"game_name": "Pac-Man", "limit": 10}

# Detect suspicious scores
@highscores-analytics detect_score_anomalies {"game_name": "Donkey Kong", "threshold_stddev": 2.5}
```
...
1. **Why MCP Matters**:
   ...
   - **Domain-Specific**: Create custom tools that understand your business logic

2. **When to Use MCP vs Extensions**:
   - **MCP Servers**: External integrations, documentation, cloud ops, domain validation
...
3. **MCP Server Types**:
   ...
   Domain-Specific    → validation, analytics, business rules (like highscores-validator)
   ```

4. **Benefits of Domain-Specific MCP Servers**:
   - **Validation**: Enforce business rules before code runs
   - **Analytics**: Compute statistics without writing SQL queries
   - **Testing**: Generate test data that matches your domain
   - **Consistency**: Reuse validation logic across frontend and backend

### Verification
```bash
# List configured MCP servers
# In Copilot Chat: @
# Should see: @context7, @azure, @highscores-validator, @highscores-analytics

# Test tool discovery
@context7 list available tools
@highscores-validator list available tools
@highscores-analytics list available tools

# Verify server health
# VS Code Output panel → GitHub Copilot Chat
# Look for: "MCP server 'context7' connected"
# Look for: "MCP server 'highscores-validator' connected"
# Look for: "MCP server 'highscores-analytics' connected"
...
# Expected: ✅ Score submission is valid

# Test analytics
@highscores-analytics calculate_player_stats {"player_name": "AAA"}
# Expected: Returns statistics from seed-data.json

# Test anomaly detection
@highscores-analytics detect_score_anomalies {}
# Expected: Analyzes all scores for statistical outliers
```
