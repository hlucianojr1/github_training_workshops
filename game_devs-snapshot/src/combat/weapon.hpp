...
public:
    ...
    enum class ReloadState {
        IDLE,           // Not reloading
        EJECTING_MAG,   // Magazine being removed
        INSERTING_MAG,  // New magazine being inserted
        CHAMBERING,     // Round being chambered (dry reload only)
        READY           // Reload complete, returning to idle
    };
...
private:
    ...
    ReloadState reload_state = ReloadState::IDLE;
    bool was_dry_reload = false;  // True if mag was empty (needs chambering)
    
    // Reload phase timing (normalized 0-1 of reload_time)
    float reload_mag_out_time = 0.3f;   // When mag eject animation should play
    float reload_chambering_time = 0.85f; // When chambering occurs (dry reload only)
    ...
    bool chambering_triggered = false; // Track if chambering event fired
...
public:
    ...
    void set_reload_chambering_time(float time) { reload_chambering_time = Math::clamp(time, 0.0f, 1.0f); }
    float get_reload_chambering_time() const { return reload_chambering_time; }
    ...
    ReloadState get_reload_state() const { return reload_state; }
    int get_reload_state_int() const { return static_cast<int>(reload_state); }
    bool is_dry_reload() const { return was_dry_reload; }
    ...
    // reload_chambering()       - Chambering animation hook (dry reload only)
    // reload_finished()
    // reload_cancelled()        - Reload was interrupted
    // reload_state_changed(int state) - State machine transition
