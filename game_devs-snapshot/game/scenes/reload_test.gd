...
func _ready():
	...
	print("Reload state: ", _state_name(weapon.get_reload_state()))
	...
	print("Is reloading: ", weapon.get_is_reloading())
	print("Reload state: ", _state_name(weapon.get_reload_state()))
	print("Is dry reload: ", weapon.is_dry_reload())
	...
	print("Reload state after cancel: ", _state_name(weapon.get_reload_state()))
	
	# Test 8: Dry reload (empty mag needs chambering)
	print("\n--- Test 8: Dry Reload (Chambering) ---")
	# Fire all remaining rounds
	while weapon.get_current_mag_ammo() > 0:
		weapon.fire()
		await get_tree().create_timer(0.1).timeout
	print("Magazine emptied: ", weapon.get_current_mag_ammo())
	weapon.start_reload()
	print("Is dry reload: ", weapon.is_dry_reload())
	print("Reload state: ", _state_name(weapon.get_reload_state()))
	await get_tree().create_timer(weapon.get_reload_time() + 0.1).timeout
	print("Reload complete, state: ", _state_name(weapon.get_reload_state()))
	print("Magazine after dry reload: ", weapon.get_current_mag_ammo())
	
	print("\n=== RELOAD TESTS COMPLETE ===")
	get_tree().quit()

## Helper to convert reload state int to name
func _state_name(state: int) -> String:
	match state:
		ReloadState.IDLE: return "IDLE"
		ReloadState.EJECTING_MAG: return "EJECTING_MAG"
		ReloadState.INSERTING_MAG: return "INSERTING_MAG"
		ReloadState.CHAMBERING: return "CHAMBERING"
		ReloadState.READY: return "READY"
