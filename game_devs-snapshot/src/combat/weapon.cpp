...
void Weapon::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("get_reload_state"), &Weapon::get_reload_state_int);
    ClassDB::bind_method(D_METHOD("is_dry_reload"), &Weapon::is_dry_reload);

    // Reload timing
    ClassDB::bind_method(D_METHOD("set_reload_chambering_time", "time"), &Weapon::set_reload_chambering_time);
    ClassDB::bind_method(D_METHOD("get_reload_chambering_time"), &Weapon::get_reload_chambering_time);
    ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "reload_chambering_time", PROPERTY_HINT_RANGE, "0.0,1.0,0.05"), "set_reload_chambering_time", "get_reload_chambering_time");
    ...
    ADD_SIGNAL(MethodInfo("reload_chambering")); // Animation hook: chambering (dry reload)
    ADD_SIGNAL(MethodInfo("reload_finished"));
    ADD_SIGNAL(MethodInfo("reload_cancelled"));
    ADD_SIGNAL(MethodInfo("reload_state_changed",
...
void Weapon::_process(double delta) {
    ...
    if (is_reloading) {
        ...
        switch (reload_state) {
            case ReloadState::EJECTING_MAG:
                // Trigger mag_out at the configured time
                if (!mag_out_triggered && reload_progress >= reload_mag_out_time) {
                    mag_out_triggered = true;
                    emit_signal("reload_mag_out");
                    reload_state = ReloadState::INSERTING_MAG;
                    emit_signal("reload_state_changed", static_cast<int>(reload_state));
                }
                break;
                
            case ReloadState::INSERTING_MAG:
                // Trigger mag_in at the configured time
                if (!mag_in_triggered && reload_progress >= reload_mag_in_time) {
                    mag_in_triggered = true;
                    emit_signal("reload_mag_in");
                    
                    // Dry reload needs chambering, tactical goes straight to READY
                    if (was_dry_reload) {
                        reload_state = ReloadState::CHAMBERING;
                    } else {
            ...
            case ReloadState::CHAMBERING:
                // Emit chambering signal at configured time (dry reload only)
                if (!chambering_triggered && reload_progress >= reload_chambering_time) {
                    chambering_triggered = true;
                    emit_signal("reload_chambering");
                    reload_state = ReloadState::READY;
                    emit_signal("reload_state_changed", static_cast<int>(reload_state));
                }
                break;
            ...
            case ReloadState::IDLE:
                // These states complete the reload
                break;
        }
        
        // Complete reload when timer finishes
...
void Weapon::start_reload() {
    ...
    // Track if this is a dry reload (magazine empty = needs chambering)
    was_dry_reload = (current_mag_ammo == 0);
    ...
    // Start in EJECTING_MAG state
    reload_state = ReloadState::EJECTING_MAG;
